...
class CustomDataset(Dataset):
    def __init__(self, seq_list, answer_list):
        self.seq_len = torch.tensor([s.shape[0] for s in seq_list]) 
        self.pad_seq = pad_sequence(
            seq_list, batch_first=True, padding_value=0)
        self.pad_ans = pad_sequence(
            answer_list, batch_first=True, padding_value=0)
...
def get_dataloader(dataset, max_step=200, batch_size = 128, mode='problem'):
    current_path = os.path.dirname(__file__)
    data_folder = os.path.join(current_path,'../data', dataset)
    ...
    pro_train, skill_train, ans_train = read_user_sequence(f'{data_folder}/train.txt', n_questions=n_questions, max_len=max_step, min_len=3)
    pro_val, skill_val, ans_val = read_user_sequence(f'{data_folder}/dev.txt', n_questions=n_questions, max_len=max_step, min_len=3)
    pro_test, skill_test, ans_test = read_user_sequence(f'{data_folder}/test.txt', n_questions=n_questions, max_len=max_step, min_len=3)
    assert mode in ['problem', 'skill']
    # if mode == 'problem':
    #     pass
    #     # pro_train, pro_test, ans_train, ans_test = train_test_split(problem, y, test_size=0.2, random_state=0)
    #     # [pro_train, ans_train], [pro_test, ans_test] = train_test_split([problem, y], split=0.8)
    # elif mode == 'skill':
    #     pass
    #     # pro_train, pro_test, ans_train, ans_test = train_test_split(skill, y, test_size=0.2, random_state=42)
    #     #  [pro_train, ans_train], [pro_test, ans_test] = train_test_split([skill, y], split=0.8)
